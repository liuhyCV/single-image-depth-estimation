...
def inference(images, reuse=False, trainable=True):
    ...
    #input [304,228], coarse6 = fc('coarse6', coarse5, [6*8*256, 4096], [4096], reuse=reuse, trainable=trainable)
    #input [576,172], coarse6 = fc('coarse6', coarse5, [4*16*256, 4096], [4096], reuse=reuse, trainable=trainable)
    '''
    coarse6 = fc('coarse6', coarse5, [4*16*256, 4096], [4096], reuse=reuse, trainable=trainable)
    coarse7 = fc('coarse7', coarse6, [4096, 3834], [3834], reuse=reuse, trainable=trainable)
    
    coarse7_output = tf.reshape(coarse7, [-1, 27, 142, 1])
